...
void Clapper::draw(GLint modelLoc, GLint textureLoc)
{

	GLfloat timeValue = fmod((WORLD_SPEED * glfwGetTime() * 2.0), 50.0f);
	GLfloat swing = std::fminf(1.0f, std::fmaxf(0.0f, (timeValue - 16.65f) / 3.5f));
	...
	model = glm::scale(model, glm::vec3(9.0f, 3.75f, 9.0f));

	model = glm::translate(model, glm::vec3(-6.0f, 0.65f, 0.0f));
	model = glm::rotate(model, glm::radians(5.0f*swing), glm::vec3(-1.0f, 0.0f, 0.0f));
	//model = glm::rotate(model, glm::radians(5.0f*fallingPercent), glm::vec3(2.0f, 0.0f, 0.0f));
	model = glm::translate(model, glm::vec3(6.0f, -0.65f, 0.0f));
