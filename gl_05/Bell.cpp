...
const int WALLS = 20;
const int VERTICES_COUNT = 12 * WALLS * 30; // splain.length * N * 30    .. + N * 2 * 15
// 9 * 5 * 30 

void Bell::createElement() {
	...
	int width, height;
	unsigned char* image = SOIL_load_image("textures/gold.jpg", &width, &height, 0, SOIL_LOAD_RGB);
...
void Bell::draw(GLint modelLoc, GLint textureLoc)
{

	GLfloat timeValue = fmod((WORLD_SPEED * glfwGetTime() * 2), 50.0f);
	GLfloat swing = std::fminf(1.0f, std::fmaxf(0.0f, (timeValue - 16.0f) / 3.5f));
	...
	model = glm::scale(model, glm::vec3(10.0f, 4.25f, 10.0f));
	model = glm::translate(model, glm::vec3(0.0f, 0.555f, 0.0f));
	//model = glm::translate(model, glm::vec3(0.11f + (0.1f*fallingPercent), -2.0f + 0.1f*fallingPercent + 0.05f*(std::sin(glm::radians(180.0f*fallingPercent))), 0.11f - (0.1f*fallingPercent)));
	//model = glm::rotate(model, glm::radians(95.0f * fallingPercent), glm::vec3(-1.0f, 0.0f, -1.0f));
