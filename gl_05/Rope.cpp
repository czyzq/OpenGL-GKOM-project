...
const int WALLS = 20;
const int VERTICES_COUNT = 4 * WALLS * 30; // splain.length * N * 30    .. + N * 2 * 15
// 9 * 5 * 30 

void Rope::createElement() {
	...
	int width, height;
	unsigned char* image = SOIL_load_image("textures/rope.jpg", &width, &height, 0, SOIL_LOAD_RGB);
...
void Rope::draw(GLint modelLoc, GLint textureLoc)
{

	GLfloat timeValue = fmod((WORLD_SPEED * glfwGetTime() * 2.0), 50.0f);

	GLfloat fallingPercent = std::fminf(1.0f, std::fmaxf(0.0f, (timeValue - 14.0f) / 3.5f));
	...
	model = glm::scale(model, glm::vec3(1.0f, 6.0f, 1.0));
	model = glm::translate(model, glm::vec3(0.0f, -0.053f,-2.25f));
	model = glm::translate(model, glm::vec3(0.0f, -0.075f*fallingPercent, 0.0f));
